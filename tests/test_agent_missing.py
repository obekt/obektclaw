...
def test_agent_mcp_load_success_and_close(mock_config, monkeypatch, tmp_path):
    ...
    mcp_config.write_text(
        json.dumps({"mcpServers": {"test": {"command": "echo", "args": ["hello"]}}})
    )
...
def test_agent_mcp_load_exception(mock_config, monkeypatch, tmp_path):
    ...
    assert any(
        "MCP failed to load: attach failed" in m.content
        for m in msgs
        if m.role == "system"
    )
...
def test_agent_run_once_max_steps(mock_config, monkeypatch):
    ...
    class MockLLM:
        def chat(self, *args, **kwargs):
            ...
            return LLMResponse(
                content="hi", tool_calls=[ToolCall("id", "test_tool", "{}")], raw="{}"
            )

    agent.llm = MockLLM()
    agent.registry.call = MagicMock(
...
def test_agent_learning_loop_exception(mock_config, monkeypatch):
    ...
    import obektclaw.post_turn

    monkeypatch.setattr(obektclaw.post_turn.TurnExtractor, "extract", mock_extract)

    # Shouldn't raise - extraction happens synchronously now
    agent.run_once("hello")
    # Check if system message was added
    msgs = agent.session.recent()
    assert any("extraction failed" in m.content for m in msgs if m.role == "system")
...
def test_agent_compose_system_prompt_facts(mock_config):
    """Test that facts can be retrieved via HybridRetriever in system prompt.

    Note: With the new automatic memory system, facts are stored in VectorMemory
    and retrieved via HybridRetriever based on semantic similarity.
    This test verifies the memory system is correctly initialized.
    """
    store = Store(mock_config.db_path)
    skills = SkillManager(store, mock_config.skills_dir, mock_config.bundled_skills_dir)
    agent = Agent(config=mock_config, store=store, skills=skills, load_mcp=False)
    ...
    # Add a fact via VectorMemory
    agent.vector_memory.add_fact(
        fact_id="test_fact_001",
        content="my_fact: my_val",
        category="general",
        confidence=0.9,
        source_turn=1,
    )

    # The HybridRetriever retrieves based on semantic similarity
    # For a query that matches the fact content, it should be retrieved
    context = agent.hybrid_retriever.retrieve_for_prompt("my_fact")

    # Verify the fact was retrieved (semantic similarity should match)
    facts = context.facts
    assert len(facts) >= 1
    # Check that our fact content appears
    found = any("my_fact" in f.get("content", "") for f in facts)
    assert found

    # Verify the context can be rendered for the prompt
    prompt_text = context.to_prompt_text()
    assert prompt_text  # Should not be empty if facts exist
