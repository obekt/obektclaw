...
def test_open():
    with (
        patch("obektclaw.__main__.Store") as mock_store,
        patch("obektclaw.__main__.SkillManager") as mock_skills,
    ):
        s, sk = _open()
...
def test_main_memory(mock_llm_cls, mock_vm_cls, mock_open, capsys, tmp_path):
    mock_store = MagicMock()
    mock_skills = MagicMock()
    mock_open.return_value = (mock_store, mock_skills)
    ...
    with patch("obektclaw.__main__.CONFIG", dummy_config):
        ...
        # search - uses VectorMemory now
        assert main(["memory", "search"]) == 1
        mock_vm = MagicMock()
        mock_vm_cls.return_value = mock_vm

        # Mock vector search results
        mock_vm.search_similar_facts.return_value = [
            {
                "content": "my_fact: my_val",
                "metadata": {"category": "general", "confidence": 0.9},
            }
        ]
        mock_vm.search_similar_entities.return_value = [
            {
                "description": "person: user (owner)",
                "metadata": {"entity_type": "person"},
            }
        ]
        mock_vm.search_similar_skills.return_value = []

        mock_store.fts_messages.return_value = [{"role": "user", "content": "msg 1"}]
        assert main(["memory", "search", "query"]) == 0
        out, _ = capsys.readouterr()
        assert "my_fact: my_val" in out
        assert "[user] msg 1" in out

        # cleanup - uses VectorMemory now
        mock_vm.get_recent_facts.return_value = []
        assert main(["memory", "cleanup"]) == 0
        out, _ = capsys.readouterr()
        assert "(no facts to clean up)" in out

        mock_vm.get_recent_facts.return_value = [
            {"id": "fact_abc123", "content": "test fact content"}
        ]
        ...
        mock_llm.chat_json.return_value = ["fact_abc123"]
        mock_vm.delete_fact = MagicMock()
        assert main(["memory", "cleanup"]) == 0
        out, _ = capsys.readouterr()
...
def test_main_memory_status(mock_open, capsys):
    ...
    mock_store.fetchone.side_effect = [
        ...
        {"c": 4},  # messages count
        {"c": 5},  # skills count
        {"ts": "2023-01-01"},  # last_session (datetime result)
    ...
    mock_store.fetchone.side_effect = [
        ...
        {"c": 20},  # messages
        {"c": 3},  # skills
        {"ts": "2024-01-01"},  # last_session
        {"journal_mode": "delete"},  # wal (non-WAL to show different mode)
    ]
