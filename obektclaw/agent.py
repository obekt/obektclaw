...
Each turn:
  1. Build a system prompt that includes the user model + automatically
     retrieved context from graph/vector memory + relevant skills.
...
NEW: Memory is fully automatic — the agent never calls memory tools.
Context is injected transparently via HybridRetriever during prompt assembly.
"""
...
- You have an automatic memory system that provides context about your preferences, 
  environment, and past learnings. This context appears in your system prompt — 
  you don't need to search for it manually.
- You have a library of skills. SEARCH them before attempting non-trivial tasks 
  using `skill_search`. Load useful skills with `skill_load`.
- When you discover a method that worked well and is likely to be reused, 
  call `skill_create` to save it. When you find a problem with an existing skill, 
  call `skill_improve` to fix it in place.
- When the user reveals a durable preference ("I always use httpx", "my prod server 
  is on Hetzner"), acknowledge it — the Learning Loop will automatically capture it.
- Prefer the smallest tool for the job. Don't run a 30s grep when a single read_file 
  would do.
...
Note: Your memory system operates automatically in the background. You don't need
to call any memory-related tools — facts and preferences are extracted and stored
...
class Agent:
    def __init__(
        ...
        *,
        ...
        # Ensure user entity exists in graph for preference relations
        ...
        log.info(
            "model=%s context_window=%d tokens=%s",
            config.llm_model,
            self.context_window,
            source,
        )
        ...
        if self._mcp_servers:
            ...
            self.session.add(
                "system", f"MCP: {len(self._mcp_servers)} server(s) loaded"
            )

    def _ensure_user_entity(self) -> None:
        """Ensure user entity exists in the knowledge graph."""
        from .memory.graph_memory import Entity

        user_entity = self.graph_memory.get_entity("entity_person_user")
        if not user_entity:
            # Create user entity for preference relations
            user_entity = Entity(
            ...
            self.graph_memory.add_entity(user_entity)

            # Sync to ChromaDB
            self.memory_sync.sync_entity_to_vector(
                entity_id="entity_person_user",
                entity_name="user",
                entity_type="person",
                description="person: user (owner)",
            )
        ...
        *,
        ...
        for _ in range(max_steps):
            ...
            for tc in resp.tool_calls:
                ...
                if result.is_error:
                    log.warning(
                        "tool_error tool=%s msg=%s", tc.name, result.content[:500]
                    )
                self.session.add(
                    "tool",
                    result.content,
        ...
        if self.run_learning_loop_flag:
            ...
            log.info(
                "turn_complete user_len=%d tool_steps=%d tokens=%d",
                len(user_text),
                tool_steps,
                self.turn_tokens,
            )

            # Run post-turn extraction synchronously - ensures facts are available
            # for subsequent turns in the same session and eliminates race conditions
            try:
                from .post_turn import TurnExtractor

                TurnExtractor(self).extract(turn)
            except Exception as e:  # noqa: BLE001
                log.warning("turn_extraction_failed error=%s", e)
                self.session.add("system", f"extraction failed: {e}")
    ...
    def switch_model(
        ...
        # Log the switch
        log.info(
            "model_switched from=%s to=%s context_window=%d overridden=%s",
    ...
    def _truncate_messages(self, messages: list[dict]) -> list[dict]:
        ...
        summary = {
            ...
            "content": (
                ...
                "context window. Key facts are preserved in automatic memory.]"
            ),
        }
    ...
    def _compose_system_prompt(self, user_text: str) -> str:
        """Compose system prompt with automatic memory retrieval.

        Uses HybridRetriever to inject relevant context without
        the agent needing to call memory tools.
        """
        parts = [SYSTEM_PROMPT.strip()]

        # User model (from SQLite user_traits table)
        traits = self.user_model.render_for_prompt()
        parts.append(f"## What I know about the user\n{traits}")

        # AUTOMATIC MEMORY RETRIEVAL (NEW)
        ...
        if context_text.strip():
            parts.append(f"## Relevant Context (automatic retrieval)\n{context_text}")

        # Log retrieval stats
        log.debug(
            "memory_retrieval facts=%d entities=%d skills=%d prefs=%d tokens=%d",
            len(retrieved_context.facts),
            len(retrieved_context.entities),
            len(retrieved_context.skills),
            len(retrieved_context.preferences),
            retrieved_context.total_tokens,
        )
        ...
        if all_skills:
            ...
            if len(all_skills) > 30:
                skills_list.append(
                    f"... and {len(all_skills) - 30} more (search with `skill_search`)"
                )
            parts.append(
                "## All available skills (load with `skill_load` if useful)\n"
                + "\n".join(skills_list)
            )

        # FTS5 recall for additional skill relevance (augments vector search)
        relevant_skills = self.skills.search(user_text, limit=4)
        if relevant_skills:
            sk_block = "\n".join(s.render_brief() for s in relevant_skills)
            parts.append("## Most relevant skills for this query\n" + sk_block)

        # FTS5 recall for session history (augments vector search)
